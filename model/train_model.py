...
# kod jak u góry alez przykładową redukcją cech nie jest tu konieczna bo  sa tylko 4 cechy
# model = Pipeline([
#     ('scaler', StandardScaler()),   # skalowanie cech
#     ('pca', PCA(n_components=3)),   # redukcja wymiarów to nie misi być
#     ('clf', SVC(kernel='linear', probability=True))   # klasyfikator SVM
# ])
...
# Zapisujemy CAŁY pipeline do pliku - przed przeiniesienim do folderu model
# Save the trained pipeline to a file
# Path("model").mkdir(exist_ok=True)
# joblib.dump(model, "model/iris_model.joblib")


# Zapisujemy CAŁY pipeline do pliku - pp przeniesiedni do folderu model
# Save the trained pipeline to a file
